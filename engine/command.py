...
#         self.api_key = LOCATION_API_KEY # HERE API key
...
#         """
#         OPENWEATHER_API_KEY = WEATHER_API_KEY  # OPENWEATHER_API_KEY
...
#         self.client = Groq(api_key=WHISPER_API_KEY) # Groq API key
...
#     @eel.expose
#     def ListenForWakeWord(self, phrase_time_limit=5, wake_word="hello nova"):
#         """
...
#     def check_up_sleep(self, timeout=40):
#         """
...
#         # Wait for response
#         response = self.check_up_sleep(timeout=40)
...
class UserManager:
    def __init__(self, state, Audio):
        self.state = state  # Reference to global app state
        self.Audio = Audio  # Reference to AudioManager for playing voice responses
        self.client = Groq(api_key="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") # Groq API key
    ...
    "Expect navigation-related words like 'navigate to', 'how far is', and 'estimated arrival time'. "
    "Egyptian area names may include: Zahraa El Maadi, Maadi, Nasr City, Dokki, Giza, Zamalek, Sheraton, New Cairo, El Rehab, Shorouk, 6 October."
    "if you find the audio empty or includes noise , leave it empty dont hear anything")
    ...
    def check_up(self, timeout=10, lang="en"):
        """
        Listen for any response from the user within the timeout.
        Filters out hallucinated or irrelevant transcriptions.
        """
        try:
            print("[Listening for response...]")
            eel.DisplayMessage("[Listening for response...]")
            ...
            if not response:
                print("[WARN] No response returned by STT.")
                return None

            response = response.strip().lower()
            print(f"[Heard]: {response}")

            # 🔒 Reject empty or garbage transcriptions
            if len(response) < 5 or response in ["", "uh", "um", "mmm", "noise", "nothing", "no"]:
                print("[DEBUG] Response too short or invalid, ignoring.")
                return None

            # 🔐 Optional: Only accept valid confirmations
            valid_keywords = ["yes", "i'm okay", "i am okay", "i'm fine", "i am fine", "i'm awake", "yes i am", "yes i can" , "aha" , "ok"]
            if any(kw in response for kw in valid_keywords):
                print("[DEBUG] Valid response detected.")
                return response

            print("[DEBUG] No confirmation found in response.")
            return None
